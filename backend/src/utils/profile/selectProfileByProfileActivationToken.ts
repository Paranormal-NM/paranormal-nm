...
export async function selectProfileByProfileActivationToken(profileActivationToken: string): Promise<Profile|null> {
    try {
        ...
        const mysqlQuery: string = "SELECT BIN_TO_UUID(profileId) as profileId, profileActivationToken profileAddressCity, profileAddressState, profileCreateTime, profileEmail, profileHash, profileUsername FROM  profile WHERE profileId = UUID_TO_BIN(:profileId)"
