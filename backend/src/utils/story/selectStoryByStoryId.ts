...
export async function selectStoryByStoryId(storyId: string) : Promise<Story|null> {
    try {
        ...
        const mySqlQuery = 'SELECT BIN_TO_UUID(storyId) AS storyId, BIN_TO_UUID (storyProfileId) AS storyProfileId, storyBody, storyCategoryId, storyLocationCity, storyLocationState, storyTitle FROM story WHERE storyId = :storyId'
